...
OUTPUT_DIR="$ROOT/auto-attack-captures"

mkdir -p "$OUTPUT_DIR"
rm -f "$OUTPUT_DIR"/*.png "$OUTPUT_DIR"/auto-attack.log 2>/dev/null || true
...
SIMCTL_CHILD_BULLETDODGE_AUTO_ATTACK_TEST=1 \
SIMCTL_CHILD_BULLETDODGE_AUTO_ATTACK_CAPTURE=1 \
...
CAPTURE_DIR="$DOCS_DIR/auto-attack-captures"
LOG_PATH="$DOCS_DIR/auto-attack.log"

for _ in {1..40}; do
  if [[ -f "$LOG_PATH" ]]; then
    cp "$LOG_PATH" "$OUTPUT_DIR"/auto-attack.log
  fi

  FILES=("$CAPTURE_DIR"/*.png)
  if (( ${#FILES[@]} >= 14 )); then
...
if [[ -d "$CAPTURE_DIR" ]]; then
  cp "$CAPTURE_DIR"/*.png "$OUTPUT_DIR"/ 2>/dev/null || true
fi
if [[ -f "$LOG_PATH" ]]; then
  cp "$LOG_PATH" "$OUTPUT_DIR"/auto-attack.log
fi
